                    ...
                    <div class="col-lg-4 col-md-6 grid-item cat1 cat2 cat3" v-for="course, cIndex in selectedCourses.length != 0?selectedCourses:courses" :key="cIndex">
                        <div class="z-gallery mb-30 border ibm-course-height">
                            <div class="z-gallery__thumb mb-20">
                                <router-link :to="'/courses/'+course.slug.replaceAll('_', '-', course.slug.toLowerCase())"><img class="img-fluid" :src="image_url+'/uploads/course'+course.thumbnail" alt=""></router-link>
...
export default {
  ...
  data(){
    ...
    watch: {
        ...
        if(this.$route.name == 'SingleCourse' || this.$route.name == 'SingleBlog' || this.$route.name == 'SingleIndustry'){

              var text = newSlug;
              var result = text.replace( /([A-Z])/g, " $1" );
              var finalResult = result.replaceAll('-', ' '); 
              return this.industryName =  finalResult.replace(/(?:^|\s)\S/g, function(a) { return a.toUpperCase(); });
    ...
    mounted(){
      ...
      if(this.$route.name == 'SingleCourse' || this.$route.name == 'SingleBlog' || this.$route.name == 'SingleIndustry'){
              ...
              var text = this.$route.params.slug.replaceAll('-', '_', this.$route.params.slug.toLowerCase()) ;
              var result = text.replace( /([A-Z])/g, " $1" );
              var finalResult = result.replaceAll('-', ' '); 
              return this.industryName =  finalResult.replace(/(?:^|\s)\S/g, function(a) { return a.toUpperCase(); });
